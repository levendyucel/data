...
# Get the list of words and definitions
words = list(word_data.keys())
definitions = list(word_data.values())

# Select a random word and its definition for the question
random_word = random.choice(words)
correct_definition = word_data[random_word]

# Display the question
st.title("Word Definition Quiz")
st.write(f"**What is the definition of:** *{random_word}*")

# Create a list of possible answers
choices = random.sample(words, 3)  # Get 3 wrong options
if random_word not in choices:
    choices.append(random_word)  # Add the correct answer to choices
random.shuffle(choices)  # Shuffle the options

# Show the choices as radio buttons
user_choice = st.radio("Choose the correct word:", choices)
...
if st.button("Submit"):
    if user_choice == random_word:
        ...
        st.error(f"Incorrect! The correct answer is: {random_word}")
